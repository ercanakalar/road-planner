...
gcloud run deploy "${SERVICE}" \
  ...
  --set-env-vars="NODE_ENV=production,UPLOAD_DIR=/mnt/uploads,CORS_ORIGINS=${CORS_ORIGINS},FRONTEND_URL=${FRONTEND_URL},SHARE_LINK_BASE_URL=${SHARE_LINK_BASE_URL},GOOGLE_REDIRECT_URL=${GOOGLE_REDIRECT_URL},GOOGLE_CLIENT_ID=${GOOGLE_CLIENT_ID},MAIL_HOST=${MAIL_HOST},MAIL_PORT=${MAIL_PORT},MAIL_USERNAME=${MAIL_USERNAME},MAIL_FROM=${MAIL_FROM}" \
  --set-secrets="DATABASE_URL=DATABASE_URL:latest,ACCESS_KEY=ACCESS_KEY:latest,REFRESH_KEY=REFRESH_KEY:latest,ROAD_SHARE_KEY=ROAD_SHARE_KEY:latest,MAIL_PASSWORD=MAIL_PASSWORD:latest,GOOGLE_CLIENT_SECRET=GOOGLE_CLIENT_SECRET:latest" \
