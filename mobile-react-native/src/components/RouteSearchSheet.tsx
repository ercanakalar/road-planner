...
interface Props {
  ...
  onClose: () => void;
  /** Show the place on the map, without changing the route. */
  onShowOnMap: (place: RoutePlace) => void;
  /** Put the place into the route, at the point it is passed. */
  ...
  ({
    ...
    return (
      <Pressable
        ...
        onPress={onPress}
        // The rows are tight so the list gets the height. The area that
        // answers a thumb stays the size a thumb is.
...
/**
 * Searching the route rather than the map.
 *
 * Everything here narrows one search — what to look for, and how far off the
 * road to look — because the route is already the boundary. That is the whole
 * difference from the search bar above the map: this one cannot answer with a
 * place on the other side of the country, however good a match it is.
 */
      ...
      <Pressable style={styles.overlay} onPress={onClose}>
        <Pressable style={styles.sheet} onPress={swallowPress}>
          ...
          <FlatList
            ...
            keyboardShouldPersistTaps='handled'
            // Read once and scrolled past, so it rides with the results
            // rather than holding a strip of the sheet open for good.
            ListHeaderComponent={
...
const createStyles = (colors: ThemeColors) =>
  StyleSheet.create({
    ...
    sheet: {
      // The controls above the list are fixed, so every point the sheet gains
      // goes to the results.
      height: '90%',
    ...
    /**
     * A ScrollView brings `flexGrow: 1` of its own, so left alone each of
     * these rows swells to swallow a quarter of the free height and the
     * filters drift apart down the sheet. Pinned to their content, the space
     * they were holding falls to the list, which is the only thing here that
     * should grow.
     */
    chipScroll: { flexGrow: 0 },
    // `gap` separates the chips within a row; the padding is the whole of the
